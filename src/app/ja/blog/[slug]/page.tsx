...
// Generate static params for all Japanese blog posts
export async function generateStaticParams() {
    const allPosts = await getAllBlogPostUids();
    return allPosts
        .filter(p => p.locale === 'ja-jp')
        .map((post) => ({
            slug: post.uid,
        }));
}
...
export default async function BlogPostPageJa({ params }: { params: Promise<{ slug: string }> }) {
    const { slug } = await params;
    const post = await getBlogPostByUid(slug, 'ja-jp');
    ...
    if (isAiNews && !onAiDomain) {
        ...
        permanentRedirect(`https://fumi-nozawa.space/ja/blog/${slug}`);
    }
    ...
    const alternatePosts = await getAlternateLocalePosts(slug, 'ja-jp');
    ...
    const breadcrumbJsonLd = generateBreadcrumbJSONLD([
        ...
        { name: isAiNews ? "AIニュース" : "ブログ", item: isAiNews ? "https://ai.fumi-nozawa.space/ja" : "https://fumi-nozawa.space/ja/blog" },
        { name: post.title, item: `${baseUrl}/ja/${isAiNews ? 'news' : 'blog'}/${post.slug}` }
    ]);
