...
const LazyImage = ({ item, index }: { item: InstagramMedia; index: number }) => {
  const { ref, inView } = useInView({
    triggerOnce: true,
    rootMargin: '400px 0px', // Increased root margin for earlier loading
      ...
      {inView ? (
        <>
          <Image
            ...
            priority={index < 12} // Prioritize more images (first 12)
            ...
            onError={(e) => {
              ...
              // Add a class to the parent to indicate error
...
export default function GalleryPage() {
  ...
  const [visibleCount, setVisibleCount] = useState(30); // Increased initial load count

  const loadMoreImages = useCallback(() => {
    setVisibleCount(prev => Math.min(prev + 20, media.length)); // Load more images at once
  }, [media.length]);

  // Intersection observer for infinite scroll with improved settings
  const { ref, inView } = useInView({
    threshold: 0,
    rootMargin: '800px 0px', // Increased root margin for earlier loading
  });

  // Load more images when bottom is reached
  useEffect(() => {
    if (inView) {
      loadMoreImages();
    }
  ...
  useEffect(() => {
    const fetchInstagramMedia = async () => {
      try {
        // Fetch Instagram media (already sorted by timestamp in the API)
