...
import { isAiNewsPost } from '@/lib/prismic-blog';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {relatedPosts.map((post) => {
                    ...
                    const postUrl = isAiNews
                        ? (locale === 'ja' ? `https://ai.fuminozawa-info.site/ja/blog/${post.slug}` : `https://ai.fuminozawa-info.site/blog/${post.slug}`)
                    ...
                    return (
                        <article key={post.id} className="group flex flex-col h-full bg-white rounded-2xl overflow-hidden border border-gray-100 hover:border-blue-200 hover:shadow-xl hover:shadow-blue-500/5 transition-all duration-300">
                            {isAiNews ? (
                                <a href={postUrl} className="flex flex-col h-full">
...
function PostContent({ post, locale }: { post: BlogPost; locale: 'en' | 'ja' }) {
    return (
        <>
            {post.featuredImage && (
